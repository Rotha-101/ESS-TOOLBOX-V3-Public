...
%% ================= USER INPUT =================
rootFolder = 'C:\Users\USER\Documents\MATLAB\test3plot'; % <<< CHANGE THIS (real time data folder)

% ===== 2nd ROOT PATH (Historical backup folder) =====
historyRootFolder = 'C:\Users\USER\Documents\MATLAB\test'; % <<< CHANGE THIS (historical data folder)
historyEssKey   = "ess";
historySmartKey = "smart";

% ===== 3rd ROOT PATH (Command / Staircase Excel folder) =====
cmdFolder = 'C:\Users\USER\Documents\MATLAB\NCC_command'; % <<< CHANGE THIS

% ===== 4th ROOT PATH (Daily cycle result Excel folder) =====
cycleParentFolder = 'C:\Users\USER\Documents\MATLAB\Cycle_Data_AllDays'; % parent folder only

dCycle = dir(fullfile(cycleParentFolder, 'SNTL_600_Cycle_Count_*'));
dCycle = dCycle([dCycle.isdir]);

if isempty(dCycle)
    error('No SNTL_600_Cycle_Count_* folder found in: %s', cycleParentFolder);
end

[~, idxLatest] = max([dCycle.datenum]);
cycleResultFolder = fullfile(dCycle(idxLatest).folder, dCycle(idxLatest).name);

% ===== 5th ROOT PATH (Remote Active Power folder) =====
remoteRootFolder = 'C:\Users\USER\Documents\MATLAB\Remote Active Power'; % <<< CHANGE THIS
remoteFolders = {'1. Plant_01','2. Plant_02','3. Plant_03'};
plantPatterns = {'*Plant_01*','*Plant_02*','*Plant_03*'};
plantTitles   = {'SWG01 (Plant 01)','SWG02 (Plant 02)','SWG03 (Plant 03)'};
...
dispatch_SOCmin = 5;
dispatch_SOCmax = 95;
dispatch_P_limit = [136 82 82];
dispatch_SOH = [1 1 1];
dispatch_Crate_dis = [0.4354 0.2721 0.2925];
dispatch_Crate_cha = [0.4354 0.2721 0.2925];

%% ================= OUTPUT =================
todayStamp = datestr(now,'yyyymmdd');

resultDir = fullfile(rootFolder, 'SNTL_600_Daily_Evaluation_Graph');
if ~exist(resultDir,'dir'), mkdir(resultDir); end

plantDir01 = fullfile(resultDir, '1. Plant_01');
plantDir02 = fullfile(resultDir, '2. Plant_02');
plantDir03 = fullfile(resultDir, '3. Plant_03');
if ~exist(plantDir01,'dir'), mkdir(plantDir01); end
if ~exist(plantDir02,'dir'), mkdir(plantDir02); end
if ~exist(plantDir03,'dir'), mkdir(plantDir03); end

allPlantDir = fullfile(resultDir, '4. Subplot_P&SOC_and_Q_U_For_All_Plants');
...
P_Q_V_F_SOC_Raw_Data.meta.rootFolder        = rootFolder;
P_Q_V_F_SOC_Raw_Data.meta.historyRootFolder = historyRootFolder;
P_Q_V_F_SOC_Raw_Data.meta.cmdFolder         = cmdFolder;
P_Q_V_F_SOC_Raw_Data.meta.cycleResultFolder = cycleResultFolder;
P_Q_V_F_SOC_Raw_Data.meta.remoteRootFolder  = remoteRootFolder;
P_Q_V_F_SOC_Raw_Data.meta.plantPatterns     = plantPatterns;
...
%% ================= LOOP THROUGH PLANTS =================
for p = 1:numel(plantPatterns)

    feedersWanted = FeedersByPlant{p};
    plantID = sprintf('%02d', p);

    dPlant = dir(fullfile(rootFolder, plantPatterns{p}));
    dPlant = dPlant([dPlant.isdir]);
    dPlant = dPlant(~ismember({dPlant.name},{'.','..'}));
    if isempty(dPlant)
        warning('No folder found for %s', plantPatterns{p});
        dbgLines = dbg(dbgLines, "WARNING: No folder found for %s", plantPatterns{p});
        continue;
    end
    plantFolder = fullfile(rootFolder, dPlant(1).name);

    files = [dir(fullfile(plantFolder,'**','*.xlsx')); dir(fullfile(plantFolder,'**','*.xls'))];
    files = files(~startsWith({files.name}, '~$'));

    if isempty(files)
        warning('No Excel files in %s', plantFolder);
        dbgLines = dbg(dbgLines, "WARNING: No Excel files in %s", plantFolder);
        continue;
    end
    ...
    dbgLines = dbg(dbgLines, "Folder: %s", plantFolder);
    dbgLines = dbg(dbgLines, "Files found: %d", numel(files));
    dbgLines = dbg(dbgLines, "=============================================");
    ...
    for k = 1:numel(files)
        ...
        try
            ...
            dbgLines = dbg(dbgLines, "WARNING: Skip file: %s | %s", files(k).name, ME.message);
        end
    end
    ...
    if ~isempty(SVData)
        ...
        [Fcol, ~] = pickOneSourceFromCandidates(freqCandidates, feedersWanted);
        ...
        [VfeederID, ~] = pickOneFeederIDForVoltage(vabCandidates, feedersWanted);
    ...
    if exist(remoteRootFolder,'dir')
        try
            [RemoteTT, RemoteRawTable, dbgLines] = loadRemoteActivePowerForPlant( ...
                remoteRootFolder, remoteFolders{p}, dayStart, dayEnd, dbgLines, DEBUG);
        catch ME
            dbgLines = dbg(dbgLines, "WARNING: Remote Active Power load failed for %s | %s", plantTitles{p}, ME.message);
        end
    else
        dbgLines = dbg(dbgLines, "WARNING: remoteRootFolder not found: %s", remoteRootFolder);
    end
    ...
    f1name = fullfile(figPlantDir, ['1. ' dataStamp '_SNTL600MWH_Frequency_ActivePower_SPPC-' plantID '.fig']);
    ...
    xlim([dayStart dayEnd]); xticks(dayStart:dtTick:dayEnd); xtickformat('HH:mm'); xtickangle(45); xlabel('Time');
    f2name = fullfile(figPlantDir, ['2. ' dataStamp '_SNTL600MWH_SOC_ActivePower_SPPC-' plantID '.fig']);
    ...
    xlim([dayStart dayEnd]); xticks(dayStart:dtTick:dayEnd); xtickformat('HH:mm'); xtickangle(45); xlabel('Time');
    f3name = fullfile(figPlantDir, ['3. ' dataStamp '_SNTL600MWH_ReactivePower_Voltage_SPPC-' plantID '.fig']);
    ...
    f4name = fullfile(figPlantDir, ['4. ' dataStamp '_SNTL600MWH_Powerflow_SPPC-' plantID '.fig']);
...
hasP3 = isfield(Result,'Plant_3') && isfield(Result.Plant_3,'M_raw') && isfield(Result.Plant_3,'remoteP');
...
if hasP1 && hasP2 && hasP3
    try
        ...
        M3 = Result.Plant_3.M_raw(:, {'SOC_pct'}); M3.Properties.VariableNames = {'SOC3_pct'};

        tt1 = Result.Plant_1.M_raw.Properties.RowTimes;
        tt2 = Result.Plant_2.M_raw.Properties.RowTimes;
        tt3 = Result.Plant_3.M_raw.Properties.RowTimes;
        ...
        R3 = timetable(tt3, Result.Plant_3.remoteP(:), 'VariableNames', {'RemoteP3_MW'});

        AllTT = synchronize(R1, R2, R3, M1, M2, M3, 'union');
        ...
        AllTT.RemoteP3_MW = fillPrev(AllTT.RemoteP3_MW);
        ...
        AllTT.SOC3_pct  = fillmissing(fillPrev(AllTT.SOC3_pct),'constant',dispatch_SOCmin);

        Pset_total_MW = AllTT.RemoteP1_MW + AllTT.RemoteP2_MW + AllTT.RemoteP3_MW;

        nAll = height(AllTT);
        Pdisp = nan(nAll,3);

        for k = 1:nAll
            SOCk  = [AllTT.SOC1_pct(k), AllTT.SOC2_pct(k), AllTT.SOC3_pct(k)];
        ...
        DispatchTT = timetable(ttAll, AllTT.RemoteP1_MW, AllTT.RemoteP2_MW, AllTT.RemoteP3_MW, ...
            Pset_total_MW, Pdisp(:,1), Pdisp(:,2), Pdisp(:,3), ...
            AllTT.SOC1_pct, AllTT.SOC2_pct, AllTT.SOC3_pct, ...
            'VariableNames', {'RemoteP_Plant01_MW','RemoteP_Plant02_MW','RemoteP_Plant03_MW', ...
            'Pset_total_MW','Pdispatch_Plant01_MW','Pdispatch_Plant02_MW','Pdispatch_Plant03_MW', ...
            'SOC1_pct','SOC2_pct','SOC3_pct'});
        ...
        dbgLines = dbg(dbgLines, "Pset source: Remote Active Power sum of Plant 01/02/03");
...
%% ================= FIGURE 5 ALL PLANTS: P & SOC + REMOTE + DISPATCH =================
...
CycleInfo = initCycleInfo(fig5Stamp);
...
f5 = figure('Color','w');
set(f5,'Units','normalized','Position',[0.05 0.08 0.92 0.82]);
tiledlayout(3,1,'TileSpacing','compact','Padding','compact');
...
for p = 1:numel(plantTitles)
    ...
    legH = pP5; legT = {'P total'};

    if hasCmd5
        pCmd5 = stairs(tt5, cmdP5, 'LineWidth', 1.6, 'Color', cmdColor);
        legH(end+1) = pCmd5; legT{end+1} = 'P command from NCC';
    end
    if hasRemote5
        pRemote5 = plot(tt5, remoteP5, '-', 'LineWidth', 1.6, 'Color', remotePowerColor);
        legH(end+1) = pRemote5; legT{end+1} = 'Remote Active Power';
    end
    if hasDisp5
        pDispLine = plot(tt5, pDisp5, '--', 'LineWidth', 1.8, 'Color', dispatchColor);
        legH(end+1) = pDispLine; legT{end+1} = 'P dispatch allocation';
    end
    hold off

    ylabel('Active Power (MW)');
    ylim(centeredYLim([M5.P_MW; cmdP5; remoteP5; pDisp5], P_center_MW, 1.05));
    ...
    legH(end+1) = pSOC5; legT{end+1} = 'SOC';
    if isgraphics(hHigh)
        legH(end+1) = hHigh;
        if highUsedBand, legT{end+1} = sprintf('High SOC hit %.1f-%.1f%%', SOC_HIGH_rng(1), SOC_HIGH_rng(2));
        else, legT{end+1} = 'Max SOC point'; end
    end
    if isgraphics(hLow)
        legH(end+1) = hLow;
        if lowUsedBand, legT{end+1} = sprintf('Low SOC hit %.1f-%.1f%%', SOC_LOW_rng(1), SOC_LOW_rng(2));
        else, legT{end+1} = 'Min SOC point'; end
    end
...
if numel(axList) >= 1 && isgraphics(axList(1))
    cycleLines = {char("Daily cycle (" + CycleInfo.dateStr + "):"); char(CycleInfo.SWG01_txt); char(CycleInfo.SWG02_txt); char(CycleInfo.SWG03_txt); char(CycleInfo.AvgDaily_txt)};
    addSecondLegendOnOverlay(axList(1), cycleLines, 'northeast');
end

if numel(axList) >= 3 && isgraphics(axList(3))
    totalCycleLines = {char("Plant Total Cycle (" + CycleInfo.dateStr + "):"); char(CycleInfo.SWG01_Total_txt); char(CycleInfo.SWG02_Total_txt); char(CycleInfo.SWG03_Total_txt); char(CycleInfo.AvgTotal_txt)};
    addSecondLegendOnOverlay(axList(3), totalCycleLines, 'northeast');
end
...
if numel(axList) >= 2 && isgraphics(axList(2))
    ...
    if pairHigh ~= "", txtHigh = "Max deviation (HIGH SOC): " + pairHigh + " = " + formatDuration(dtHigh);
    else, txtHigh = "Max deviation (HIGH SOC): (not enough data)"; end
    if pairLow ~= "", txtLow = "Max deviation (LOW SOC): " + pairLow + " = " + formatDuration(dtLow);
    else, txtLow = "Max deviation (LOW SOC): (not enough data)"; end
...
f5name = fullfile(allPlantDir, ['1. ' fig5Stamp '_SNTL600MWH_ActivePower_SOC_AllPlants.fig']);
...
f6name = fullfile(allPlantDir, ['2. ' fig6Stamp '_SNTL600MWH_ReactivePower_Voltage_AllPlants.fig']);
savefig(f6, f6name); close(f6);

%% ================= ADD HISTORICAL DATA =================
Historical = struct();
Historical.meta.historyRootFolder = historyRootFolder;
Historical.meta.historyEssKey = historyEssKey;
Historical.meta.historySmartKey = historySmartKey;

if exist(historyRootFolder,'dir')
    try
        [Historical, dbgLines] = loadHistoricalFolder(historyRootFolder, historyEssKey, historySmartKey, dbgLines, DEBUG);
    catch ME
        dbgLines = dbg(dbgLines, "WARNING: Historical load failed: %s", ME.message);
    end
else
    dbgLines = dbg(dbgLines, "WARNING: historyRootFolder not found: %s", historyRootFolder);
end
...
matFile = fullfile(resultDir, ['1. SNTL600MWH_', dataStampGlobal, '_data.mat']);
...
if ~isempty(DispatchTT)
    try
        Tdispatch = timetable2table(DispatchTT, 'ConvertRowTimes', true);
        Tdispatch.Properties.VariableNames{1} = 'Time';
        writetable(Tdispatch, outXlsx, 'Sheet', 'Realtime_Dispatch');
        fprintf('[INFO] Remote dispatch exported to sheet: Realtime_Dispatch\n');
    catch ME
        warning('Failed to write realtime dispatch sheet: %s', ME.message);
    end
end
...
function [CmdTT, dbgLines] = loadCmdFolder(cmdFolder, dbgLines, DEBUG)
    ...
    for k = 1:numel(cmdFiles)
        ...
        try
            ...
            realP3 = getReal("SWG03 P(MW)"); realQ3 = getReal("SWG03 Q(MVAR)");
            if realDatetime=="" || any([realP1 realQ1 realP2 realQ2 realP3 realQ3]==""), continue; end
            realSOC1 = getReal("SWG01 SOC(%)"); realSOC2 = getReal("SWG02 SOC(%)"); realSOC3 = getReal("SWG03 SOC(%)");
            ...
            Tout.SWG03_MW = T.(realP3); Tout.SWG03_Mvar = T.(realQ3);
            ...
            if realSOC3 ~= "", Tout.SWG03_SOC = T.(realSOC3); else, Tout.SWG03_SOC = NaN(height(T),1); end
            Tall = appendUnionSmart(Tall, Tout); nRead = nRead + 1;
            if DEBUG, dbgLines = dbg(dbgLines, "  [OK CMD] %s", fp); end
        catch ME
            dbgLines = dbg(dbgLines, "  [SKIP CMD] %s | %s", fp, ME.message);
        end
    end
    ...
    colsToNum = ["SWG01_MW","SWG02_MW","SWG03_MW","SWG01_Mvar","SWG02_Mvar","SWG03_Mvar","SWG01_SOC","SWG02_SOC","SWG03_SOC"];
    for c = colsToNum
        if ismember(c, string(Tall.Properties.VariableNames)), Tall.(c) = fillPrev(safeToDouble(Tall.(c))); end
    end
    CmdTT = timetable(Tall.Time, Tall.SWG01_MW, Tall.SWG02_MW, Tall.SWG03_MW, Tall.SWG01_Mvar, Tall.SWG02_Mvar, Tall.SWG03_Mvar, Tall.SWG01_SOC, Tall.SWG02_SOC, Tall.SWG03_SOC, ...
        'VariableNames', {'SWG01_MW','SWG02_MW','SWG03_MW','SWG01_Mvar','SWG02_Mvar','SWG03_Mvar','SWG01_SOC','SWG02_SOC','SWG03_SOC'});
...
function [Historical, dbgLines] = loadHistoricalFolder(historyRootFolder, essKey, smartKey, dbgLines, DEBUG)
    allFiles = [dir(fullfile(historyRootFolder,'**','*.xlsx')); dir(fullfile(historyRootFolder,'**','*.xls')); dir(fullfile(historyRootFolder,'**','*.csv'))];
    allFiles = allFiles(~startsWith({allFiles.name}, '~$'));
    fpaths = string(fullfile({allFiles.folder},{allFiles.name}))'; foldersLow = lower(string({allFiles.folder})');
    isESS = contains(foldersLow, lower(essKey)); isSmart = contains(foldersLow, lower(smartKey)); isOther = ~(isESS | isSmart);
    [Historical.ESS_data, dbgLines] = readFileGroup(fpaths(isESS), "ESS_data", dbgLines, DEBUG);
    [Historical.SmartLog, dbgLines] = readFileGroup(fpaths(isSmart), "SmartLog", dbgLines, DEBUG);
    [Historical.Other, dbgLines] = readFileGroup(fpaths(isOther), "Other", dbgLines, DEBUG);
end

function [S, dbgLines] = readFileGroup(fileList, groupName, dbgLines, DEBUG)
    S = struct(); S.files = fileList(:); S.tables = cell(numel(fileList),1); S.readOk = false(numel(fileList),1); S.error = strings(numel(fileList),1);
    for i = 1:numel(fileList)
        fp = fileList(i); [~,~,ext] = fileparts(fp);
        try
            if strcmpi(ext,'.csv'), T = readtable(fp, 'TextType','string'); else, T = readtable(fp, 'PreserveVariableNames', true); end
            S.tables{i} = T; S.readOk(i) = true;
            if DEBUG, dbgLines = dbg(dbgLines, "  [OK Historical %s] %s", groupName, fp); end
        catch ME
            S.tables{i} = []; S.readOk(i) = false; S.error(i) = string(ME.message);
            dbgLines = dbg(dbgLines, "  [SKIP Historical %s] %s | %s", groupName, fp, ME.message);
        end
    end
end

function [RemoteTT, RemoteRawTable, dbgLines] = loadRemoteActivePowerForPlant(remoteRootFolder, remoteFolderName, dayStart, dayEnd, dbgLines, DEBUG)
    RemoteTT = timetable(); RemoteRawTable = table(); plantFolder = fullfile(remoteRootFolder, remoteFolderName);
    if ~exist(plantFolder,'dir'), dbgLines = dbg(dbgLines, "WARNING: Remote Active Power folder not found: %s", plantFolder); return; end
    files = [dir(fullfile(plantFolder,'**','*.xlsx')); dir(fullfile(plantFolder,'**','*.xls'))]; files = files(~startsWith({files.name}, '~$'));
    if isempty(files), dbgLines = dbg(dbgLines, "WARNING: No Remote Active Power Excel found in: %s", plantFolder); return; end
    Tall = table();
    for k = 1:numel(files)
        ...
        try
            ...
            if DEBUG, dbgLines = dbg(dbgLines, "  [OK Remote] %s", fp); end
        catch ME
            dbgLines = dbg(dbgLines, "  [SKIP Remote] %s | %s", fp, ME.message);
        end
    end
    if isempty(Tall), dbgLines = dbg(dbgLines, "WARNING: No usable Remote Active Power data in: %s", plantFolder); return; end
    Tall = sortrows(Tall, "Time"); Tall = Tall(Tall.Time >= dayStart & Tall.Time <= dayEnd, :);
    if isempty(Tall), dbgLines = dbg(dbgLines, "WARNING: Remote Active Power data not in date range: %s", plantFolder); return; end
...
function [tHit, yHit] = detectFirstHitInRange(tt, socVec, rng2, tAfter)
    tHit = NaT; yHit = NaN; if isempty(tt) || isempty(socVec), return; end
    socVec = socVec(:); tt = tt(:); inBand = socVec >= rng2(1) & socVec <= rng2(2) & ~isnan(socVec);
    if ~isnat(tAfter), inBand = inBand & (tt > tAfter); end
    idx = find(inBand, 1, 'first'); if isempty(idx), return; end
    tHit = tt(idx); yHit = socVec(idx);
end

function [tHit, yHit] = detectMaxSOCPoint(tt, socVec)
    tHit = NaT; yHit = NaN; if isempty(tt) || isempty(socVec), return; end
    socVec = socVec(:); tt = tt(:); valid = ~isnan(socVec) & ~isnat(tt); if ~any(valid), return; end
    socValid = socVec(valid); ttValid = tt(valid); yMax = max(socValid); idx = find(socValid == yMax, 1, 'first'); if isempty(idx), return; end
    tHit = ttValid(idx); yHit = socValid(idx);
end

function [tHit, yHit] = detectMinSOCPoint(tt, socVec, tAfter)
    tHit = NaT; yHit = NaN; if isempty(tt) || isempty(socVec), return; end
    socVec = socVec(:); tt = tt(:); valid = ~isnan(socVec) & ~isnat(tt); if ~isnat(tAfter), valid = valid & (tt > tAfter); end
    if ~any(valid), return; end
    socValid = socVec(valid); ttValid = tt(valid); yMin = min(socValid); idx = find(socValid == yMin, 1, 'first'); if isempty(idx), return; end
    tHit = ttValid(idx); yHit = socValid(idx);
end

function [tLow, yLow, usedBand] = detectLowSOCAfterHigh(tt, socVec, lowRng, tHigh)
    tLow = NaT; yLow = NaN; usedBand = false; if isempty(tt) || isempty(socVec), return; end
    if ~isnat(tHigh)
        [tBand, yBand] = detectFirstHitInRange(tt, socVec, lowRng, tHigh);
        if ~isnat(tBand), tLow = tBand; yLow = yBand; usedBand = true; return; end
        [tMinAfter, yMinAfter] = detectMinSOCPoint(tt, socVec, tHigh);
        if ~isnat(tMinAfter), tLow = tMinAfter; yLow = yMinAfter; return; end
    end
    [tMin, yMin] = detectMinSOCPoint(tt, socVec, NaT);
    if ~isnat(tMin), tLow = tMin; yLow = yMin; end
end

function [pairLabel, maxDelta] = maxPairDeviation(tArr)
    pairLabel = ""; maxDelta = seconds(NaN); valid = ~isnat(tArr); if sum(valid) < 2, return; end
    durs = [abs(tArr(1)-tArr(2)) abs(tArr(2)-tArr(3)) abs(tArr(3)-tArr(1))]; labels = ["SWG01-SWG02","SWG02-SWG03","SWG03-SWG01"];
    durSec = seconds(durs); durSec(isnan(durSec)) = -Inf; [mx, i] = max(durSec); if isinf(mx) && mx < 0, return; end
    pairLabel = labels(i); maxDelta = seconds(mx);
end

function s = formatDuration(d)
    if isa(d,'duration'), totalSec = seconds(d); else, totalSec = d; end
    if isnan(totalSec) || isinf(totalSec), s = "(n/a)"; return; end
    totalSec = round(totalSec); mm = floor(totalSec/60); ss = totalSec - mm*60; s = sprintf('%dm %02ds', mm, ss);
end

function lg2 = addSecondLegendOnOverlay(axBase, linesCell, loc)
    if nargin < 3, loc = 'best'; end
    fig = ancestor(axBase,'figure');
    ax2 = axes('Parent', fig, 'Units', axBase.Units, 'Position', axBase.Position, 'Color','none', 'XTick', [], 'YTick', [], 'XColor','none', 'YColor','none', 'Box','off', 'Visible','off', 'HitTest','off', 'HandleVisibility','off');
    hold(ax2,'on'); n = numel(linesCell); h = gobjects(1,n); for k = 1:n, h(k) = plot(ax2, NaN, NaN, 'LineStyle','none', 'Marker','none'); end; hold(ax2,'off');
    lg2 = legend(ax2, h, linesCell, 'Location', loc); lg2.Interpreter = 'tex'; lg2.FontSize = 9; lg2.Box = 'on'; lg2.AutoUpdate = 'off';
    set(fig,'SizeChangedFcn',@(src,evt) set(ax2,'Position', axBase.Position)); uistack(ax2, 'top');
end

function CycleInfo = initCycleInfo(figStamp)
    CycleInfo = struct(); CycleInfo.hasData = false; CycleInfo.dateStr = string(datetime(figStamp,'InputFormat','yyyyMMdd','Format','yyyy-MM-dd'));
    CycleInfo.SWG01 = NaN; CycleInfo.SWG02 = NaN; CycleInfo.SWG03 = NaN; CycleInfo.AverageDailyCycle = NaN;
    CycleInfo.SWG01_Total = NaN; CycleInfo.SWG02_Total = NaN; CycleInfo.SWG03_Total = NaN; CycleInfo.AverageTotalPlantCycle = NaN;
    CycleInfo.SWG01_txt = "Cycle_Plant 01 = (no data)"; CycleInfo.SWG02_txt = "Cycle_Plant 02 = (no data)"; CycleInfo.SWG03_txt = "Cycle_Plant 03 = (no data)"; CycleInfo.AvgDaily_txt = "Average Daily Cycle = (no data)";
    CycleInfo.SWG01_Total_txt = "Plant 01 Total Cycle = (no data)"; CycleInfo.SWG02_Total_txt = "Plant 02 Total Cycle = (no data)"; CycleInfo.SWG03_Total_txt = "Plant 03 Total Cycle = (no data)"; CycleInfo.AvgTotal_txt = "Average Total Plant Cycle = (no data)";
end

function [CycleInfo, dbgLines] = loadDailyCycleInfo(cycleResultFolder, fig5Stamp, dbgLines, DEBUG)
    CycleInfo = initCycleInfo(fig5Stamp);
    targetDate = datetime(fig5Stamp,'InputFormat','yyyyMMdd','Format','yyyy-MM-dd'); targetDateStr = string(targetDate,'yyyy-MM-dd');
    cycleFiles = [dir(fullfile(cycleResultFolder, 'SPPC_Extracted_EquivalentCycles_AllDays*.xlsx')); dir(fullfile(cycleResultFolder, 'Daily_SWG_Cycle_Result*.xlsx'))];
    cycleFiles = cycleFiles(~startsWith({cycleFiles.name}, '~$'));
    if isempty(cycleFiles), dbgLines = dbg(dbgLines, "WARNING: No daily cycle result Excel found in: %s", cycleResultFolder); return; end
    pickedFile = ""; T = table();
    for k = 1:numel(cycleFiles)
        fp = fullfile(cycleFiles(k).folder, cycleFiles(k).name);
        try
            [~, sheets] = xlsfinfo(fp);
            if any(strcmpi(sheets, 'Daily_SWG_Cycle_Result')), T = readtable(fp, 'Sheet', 'Daily_SWG_Cycle_Result', 'PreserveVariableNames', true); pickedFile = fp; break; end
        catch
        end
    end
    if isempty(T), dbgLines = dbg(dbgLines, "WARNING: Cannot read Daily_SWG_Cycle_Result sheet from folder: %s", cycleResultFolder); return; end
    vars = string(T.Properties.VariableNames); vlow = lower(vars);
    dateCol = pickCol(vars, vlow, "datadate"); swg1DailyCol = pickCol(vars, vlow, "swg01_dailyreached"); swg2DailyCol = pickCol(vars, vlow, "swg02_dailyreached"); swg3DailyCol = pickCol(vars, vlow, "swg03_dailyreached"); avgDailyCol = pickCol(vars, vlow, "average_daily_cycle");
    swg1TotalCol = pickCol(vars, vlow, "swg01_totalcycle"); swg2TotalCol = pickCol(vars, vlow, "swg02_totalcycle"); swg3TotalCol = pickCol(vars, vlow, "swg03_totalcycle"); avgTotalCol = pickCol(vars, vlow, "average_total_plant_cycle");
    if dateCol == "" || swg1DailyCol == "" || swg2DailyCol == "" || swg3DailyCol == "", dbgLines = dbg(dbgLines, "WARNING: Required daily cycle columns not found in file: %s", pickedFile); return; end
    rowDate = strip(string(T.(dateCol))); idxRow = find(rowDate == targetDateStr, 1, 'first');
    if isempty(idxRow)
        try, dt = parseDateFlex(T.(dateCol)); idxRow = find(string(dt,'yyyy-MM-dd') == targetDateStr, 1, 'first'); catch, idxRow = []; end
    end
    if isempty(idxRow), dbgLines = dbg(dbgLines, "WARNING: No matching cycle row for date %s in file: %s", targetDateStr, pickedFile); return; end
    CycleInfo.hasData = true;
    CycleInfo.SWG01 = safeScalarNum(T.(swg1DailyCol)(idxRow)); CycleInfo.SWG02 = safeScalarNum(T.(swg2DailyCol)(idxRow)); CycleInfo.SWG03 = safeScalarNum(T.(swg3DailyCol)(idxRow));
    if avgDailyCol ~= "", CycleInfo.AverageDailyCycle = safeScalarNum(T.(avgDailyCol)(idxRow)); else, CycleInfo.AverageDailyCycle = mean([CycleInfo.SWG01 CycleInfo.SWG02 CycleInfo.SWG03], 'omitnan'); end
    if swg1TotalCol ~= "", CycleInfo.SWG01_Total = safeScalarNum(T.(swg1TotalCol)(idxRow)); end
    if swg2TotalCol ~= "", CycleInfo.SWG02_Total = safeScalarNum(T.(swg2TotalCol)(idxRow)); end
    if swg3TotalCol ~= "", CycleInfo.SWG03_Total = safeScalarNum(T.(swg3TotalCol)(idxRow)); end
    if avgTotalCol ~= "", CycleInfo.AverageTotalPlantCycle = safeScalarNum(T.(avgTotalCol)(idxRow)); else, CycleInfo.AverageTotalPlantCycle = mean([CycleInfo.SWG01_Total CycleInfo.SWG02_Total CycleInfo.SWG03_Total], 'omitnan'); end
    CycleInfo.SWG01_txt = formatCycleLegendLine("Plant 01", CycleInfo.SWG01); CycleInfo.SWG02_txt = formatCycleLegendLine("Plant 02", CycleInfo.SWG02); CycleInfo.SWG03_txt = formatCycleLegendLine("Plant 03", CycleInfo.SWG03); CycleInfo.AvgDaily_txt = formatCycleLegendLine("Average Daily Cycle", CycleInfo.AverageDailyCycle);
    CycleInfo.SWG01_Total_txt = sprintf('Plant 01 Total Cycle = %.6f', CycleInfo.SWG01_Total); CycleInfo.SWG02_Total_txt = sprintf('Plant 02 Total Cycle = %.6f', CycleInfo.SWG02_Total); CycleInfo.SWG03_Total_txt = sprintf('Plant 03 Total Cycle = %.6f', CycleInfo.SWG03_Total); CycleInfo.AvgTotal_txt = sprintf('Average Total Plant Cycle = %.6f', CycleInfo.AverageTotalPlantCycle);
    if DEBUG, dbgLines = dbg(dbgLines, "[INFO] Daily cycle result loaded from: %s", pickedFile); end
end

function x = safeScalarNum(v)
    if isnumeric(v), x = double(v(1)); else, x = str2double(string(v(1))); end
    if isnan(x), x = NaN; end
end

function txt = formatCycleLegendLine(plantLabel, cycleVal)
    if isnan(cycleVal), txt = ['Cycle\_' char(plantLabel) ' = (no data)']; return; end
    [statusText, statusIcon] = classifyDailyCycleStatus(cycleVal);
    txt = sprintf('Cycle\\_%s = %.3f  -> %s %s', plantLabel, cycleVal, statusIcon, statusText);
end

function [statusText, statusIcon] = classifyDailyCycleStatus(cycleVal)
    if isnan(cycleVal), statusText = 'No data'; statusIcon = '\color[rgb]{0.5 0.5 0.5}\bullet\color{black}';
    elseif cycleVal < 0.5, statusText = 'Take action'; statusIcon = '\color[rgb]{0.85 0.20 0.25}\bullet\color{black}';
    elseif cycleVal >= 0.5 && cycleVal < 0.8, statusText = 'Warning'; statusIcon = '\color[rgb]{0.90 0.70 0.15}\bullet\color{black}';
    else, statusText = 'Normal'; statusIcon = '\color[rgb]{0.20 0.75 0.45}\bullet\color{black}'; end
end

function dt = parseDateFlex(x)
    dt = NaT(size(x));
    if isdatetime(x), dt = x; return; end
    if isnumeric(x), try, dt = datetime(x, 'ConvertFrom','excel', 'Format','yyyy-MM-dd'); catch, end; return; end
    x = strip(string(x)); fmts = ["yyyy-MM-dd";"dd-MMM-yy";"dd-MMM-yyyy";"dd/MM/yyyy";"MM/dd/yyyy";"yyyyMMdd"];
    for f = 1:numel(fmts)
        try, tf = datetime(x, 'InputFormat', fmts(f), 'Format','yyyy-MM-dd'); ok = ~isnat(tf); dt(ok) = tf(ok); if all(~isnat(dt)), return; end; catch, end
    end
    try, dt = datetime(x, 'Format','yyyy-MM-dd'); catch, end
end

function [Pi, mode] = alloc_with_limits(Pset, SOCc, SOH, SOCmin, SOCmax, Crate_dis, Crate_cha, P_limit)
    Pi = zeros(1,3);
    if Pset > 0
        w = (SOCc - SOCmin) .* SOH .* Crate_dis; mode = "Discharge";
    elseif Pset < 0
        w = (SOCmax - SOCc) .* SOH .* Crate_cha; mode = "Charge";
    else
        mode = "Idle"; return;
    end
    if sum(w) <= 0, Pi(:) = 0; return; end
    signP = sign(Pset); Pmag = abs(Pset); active = true(1,3); Pi_mag = zeros(1,3); remaining = Pmag;
    for iter = 1:3
        if remaining <= 1e-9, break; end
        if sum(w(active)) <= 0, break; end
        alloc = zeros(1,3); alloc(active) = remaining * (w(active) / sum(w(active)));
        for i = 1:3
            if ~active(i), continue; end
            cap = P_limit(i) - Pi_mag(i);
            if cap <= 1e-12, active(i) = false; continue; end
            if alloc(i) >= cap, Pi_mag(i) = Pi_mag(i) + cap; active(i) = false; else, Pi_mag(i) = Pi_mag(i) + alloc(i); end
        end
...
function col = pickCol(vars, vlow, key)
    col = ""; key = lower(string(key)); idx = find(contains(vlow, key), 1);
    if ~isempty(idx), col = vars(idx); end
end