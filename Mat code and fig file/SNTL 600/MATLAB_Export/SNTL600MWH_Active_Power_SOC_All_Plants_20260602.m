...
try
    ...
    if isfield(data, 'dailyCycle') && isfield(data.dailyCycle, 'plant3')
        val = data.dailyCycle.plant3;
        cycleLines(end+1) = "  SWG03: " + sprintf('%.2f', val);
        sumDaily = sumDaily + val; countDaily = countDaily + 1;
    end
    ...
    if isfield(data, 'totalCycle') && isfield(data.totalCycle, 'plant3')
        val = data.totalCycle.plant3;
        totalCycleLines(end+1) = "  SWG03: " + sprintf('%.2f', val);
        sumTotal = sumTotal + val; countTotal = countTotal + 1;
    end
...
if SAVE_FIG_AND_CLOSE
    savefig(fig, 'SNTL600MWH_Active_Power_SOC_All_Plants_20260602.fig');
